...
def download():
    ...
    if config.RAW_FILE.exists():
        print(f"[OK] Dataset already exists at: {config.RAW_FILE}")
    ...
    print(f"  [OK] Downloaded ({zip_path.stat().st_size / 1024:.0f} KB)")
    ...
    if extracted.exists():
        ...
        print(f"  [OK] Renamed to: {config.RAW_FILE.name}")
    elif config.RAW_FILE.exists():
        print(f"  [OK] File already at: {config.RAW_FILE.name}")
    else:
    ...
    print(f"\n  [OK] Verified: {line_count:,} lines in {config.RAW_FILE.name}")
